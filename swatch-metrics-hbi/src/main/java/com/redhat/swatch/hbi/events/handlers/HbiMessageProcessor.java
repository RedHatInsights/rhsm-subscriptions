...
public class HbiMessageProcessor {
  ...
  @Transactional
  public List<Event> process(HbiEvent hbiEvent) {
    log.info("Processing host event from HBI - {}", hbiEvent);
    HbiEventHandler<?> hbiEventHandler = this.handlers.get(hbiEvent.getClass());
    if (hbiEventHandler.skipEvent(hbiEvent)) {
      log.debug("HBI event {} will be filtered.", hbiEvent);
      return List.of();
    }
    return hbiEventHandler.handleEvent(hbiEvent);
  }
