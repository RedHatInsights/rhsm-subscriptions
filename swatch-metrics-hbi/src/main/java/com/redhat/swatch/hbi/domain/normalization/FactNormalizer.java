...
/**
 * The FactNormalizer class is responsible for processing and normalizing various fact objects
 * associated with a given host. The facts are extracted from multiple sources, such as RHSM,
 * Satellite, QPC, and system profile data, and transformed into a standardized format suitable for
 * downstream processing and analysis.
 *
 * <p>This class uses injected dependencies to obtain system configuration, manage host information,
 * and record logging details. It performs conditional fact processing based on criteria such as
 * synchronization timestamps, cloud provider types, virtualization checks, and more.
 *
 * <p>Primary functions of the FactNormalizer class include: - Extracting and validating fact
 * attributes from host data. - Determining properties such as instance IDs, hardware type,
 * virtualization status, cloud provider type, and hypervisor UUIDs. - Normalizing product tags and
 * IDs based on input data and system profile mappings. - Handling optional fact inputs with
 * fallback logic and conditions such as skipping RHSM facts. - Constructing and returning a
 * normalized facts object that encapsulates host properties.
 *
 * <p>This class is annotated with Spring or CDI annotations (`@ApplicationScoped`, `@Inject`) for
 * dependency injection management and uses a logger (`@Slf4j`) for traceability/logging.
 */
@Slf4j
@ApplicationScoped
public class FactNormalizer {
  ...
  public FactNormalizer(
      ApplicationClock clock, ApplicationProperties settings, HbiHostManager hbiHostManager) {
    this.clock = clock;
  ...
  public NormalizedFacts normalize(Host host) {
    ...
    boolean isHypervisor = hbiHostManager.isHypervisor(orgId, subscriptionManagerId);
    boolean isUnmappedGuest =
        isVirtual
            && StringUtils.isNotEmpty(hypervisorUuid)
            && !hbiHostManager.isKnownHost(orgId, hypervisorUuid);
  ...
  private boolean hostUnregistered(OffsetDateTime lastSync) {
    ...
    return lastSync.isBefore(clock.startOfToday().minus(settings.getHostLastSyncThreshold()));
  }
