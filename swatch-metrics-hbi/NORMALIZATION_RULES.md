...
This determines what product tags to assign to a certain host. All sources below are touched in ProductNormalizer and their resulting product tags are merged into one host event. The order between the first three sources does not affect the outcome. When QPC reports RHEL, the generic `RHEL` tag is always added; architecture-specific QPC tags are only added when the system profile has no installed products.
...
| QpcFacts `rh_products_installed` | When QPC reports RHEL: always adds generic `RHEL` tag; architecture-specific tag (see below) only when system profile has no installed products |

### QPC Arch-to-variant Mapping

When QPC reports RHEL, the generic `RHEL` tag is always added. Architecture-specific tags are added only when the system profile has no installed products:
