...
# Test script for reproducing the cascading deductions bug using Kafka events
# This sends events through Kafka to trigger the conflict resolution process

echo "=== Kafka Events Cascading Deductions Bug Test ==="
echo
...
# Create initial conflicting events in database
echo "Creating initial conflicting events in database..."
echo
...
echo
echo "Initial events created. Now sending new conflicting events through Kafka..."
echo

# Send new events through Kafka that should conflict with existing ones
echo "Sending new event through Kafka: vCPUs = 3.0"
curl -X 'POST' "http://localhost:8003/api/rhsm-subscriptions/v1/internal/rpc/tally/events" \
  -H 'Content-Type: application/json' \
  -H 'x-rh-swatch-psk: placeholder' \
  -d "[{
    \"event_id\": \"kafka-event-1\",
    \"org_id\": \"$ORG_ID\",
    \"timestamp\": \"$TIMESTAMP\",
    \"instance_id\": \"$INSTANCE_ID\",
    \"event_type\": \"snapshot\",
...
echo "Sending another new event through Kafka: vCPUs = 4.0"
curl -X 'POST' "http://localhost:8003/api/rhsm-subscriptions/v1/internal/rpc/tally/events" \
  -H 'Content-Type: application/json' \
  -H 'x-rh-swatch-psk: placeholder' \
  -d "[{
    \"event_id\": \"kafka-event-2\",
    \"org_id\": \"$ORG_ID\",
    \"timestamp\": \"$TIMESTAMP\",
    \"instance_id\": \"$INSTANCE_ID\",
    \"event_type\": \"snapshot\",
...
echo
echo "Kafka events sent. Waiting for processing..."
sleep 5
...
# Analysis
if [ "$TOTAL_EVENTS" -gt 5 ] && [ "$DEDUCTION_EVENTS" -gt 0 ]; then
...
elif [ "$TOTAL_EVENTS" -gt 5 ]; then
    echo "❌ BUG DETECTED: Multiple events found without proper conflict resolution"
    echo "   Expected: 1 event, Found: $TOTAL_EVENTS events"
else
