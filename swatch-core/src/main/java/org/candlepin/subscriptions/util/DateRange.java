...
public class DateRange {
  ...
  /**
   * Create a DateRange object from a set of strings formatted in the ISO Offset Date Time format as
   * seen in {@link DateTimeFormatter#ISO_OFFSET_DATE_TIME}
   *
   ...
   * Note that the strings will be parsed with {@link OffsetDateTime#parse(CharSequence,
   * DateTimeFormatter)} so a compatible formatter is required to avoid runtime exceptions.
   *
   * @param start a string with the start date
   * @param end a string with the end date
