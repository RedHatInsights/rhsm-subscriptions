...
/**
 * This is a Spring version of ResteasyServletInitializer. It does not register the servlets though,
 * that is done by {@link ResteasyApplicationBuilder} It only finds the JAX-RS Application classes
 * (by scanning the classpath), and the JAX-RS Path and Provider annotated Spring beans, and then
 * register the Spring bean definitions that represent each servlet registration.
 *
 * @author Fabio Carvalho (facarvalho@paypal.com or fabiocarvalho777@gmail.com)
 */
public class ResteasyBeanProcessorTomcat extends ResteasyResourcesFinder
  ...
  /**
   * Register a default JAX-RS application, in case no other is present in the application. Read
   * section 2.3.2 in JAX-RS 2.0 specification.
   *
  ...
  private void registerDefaultJaxrsApp(ConfigurableListableBeanFactory beanFactory) {
    ...
    logger.info(
        "No JAX-RS Application classes have been found. A default, one mapped to '{}', will be registered.",
        path);
   ...
   *
   * @param applicationClass Spring Boot Application class
   * @param path servlet path
   * @return a Servlet bean definition for the given JAX-RS application
   */
  private GenericBeanDefinition createApplicationServlet(
      Class<? extends Application> applicationClass, String path) {
