...
public class UsageSnapshotProducer {
    ...
    private final KafkaTemplate<String, TallySummary> kafkaTemplate;
    ...
    public UsageSnapshotProducer(TallySnapshotRepository tallyRepo, ApplicationClock clock,
        ProductProfileRegistry registry, KafkaTemplate<String, TallySummary> kafkaTemplate,
    ...
    public void produceSnapshotsFromCalculations(Collection<String> accounts,
        ...
        Map<String, List<TallySnapshot>> newAndUpdatedSnapshots = rollers
    ...
    private void produceTallySummaryMessages(Map<String, List<TallySnapshot>> newAndUpdatedSnapshots) {
        newAndUpdatedSnapshots.entrySet().stream()
            .map(entry -> createTallySummary(entry.getKey(), entry.getValue()))
            .forEach(tallySummary -> kafkaTemplate.send(tallySummaryTopic, tallySummary));
    }

    private TallySummary createTallySummary(String accountNumber, List<TallySnapshot> tallySnapshots) {
        List<org.candlepin.subscriptions.json.TallySnapshot> mappedSnapshots = tallySnapshots.stream()
    ...
    private org.candlepin.subscriptions.json.TallySnapshot mapTallySnapshot(TallySnapshot tallySnapshot) {
        return new org.candlepin.subscriptions.json.TallySnapshot()
            .withGranularity(org.candlepin.subscriptions.json.TallySnapshot.Granularity.fromValue(tallySnapshot.getGranularity().getValue()))
            .withId(tallySnapshot.getId())
            .withProductId(tallySnapshot.getProductId())
            .withSnapshotDate(tallySnapshot.getSnapshotDate())
            .withSla(org.candlepin.subscriptions.json.TallySnapshot.Sla.fromValue(tallySnapshot.getServiceLevel().getValue()))
    ...
    private List<TallyMeasurement> mapMeasurements(Map<TallyMeasurementKey, Double> tallyMeasurements) {
        return tallyMeasurements.entrySet().stream()
            ...
            .withHardwareMeasurementType(entry.getKey().getCategory().toString())
