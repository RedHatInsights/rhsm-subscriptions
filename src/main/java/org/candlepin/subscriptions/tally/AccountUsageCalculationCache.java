...
/**
 * An object that temporarily caches instances of {@link AccountUsageCalculation} objects and Maps
 * them by {@link OffsetDateTime}. This object is used to cache any current usage calculations for
 * an associated usage timestamp (Event.timestamp) while the hourly tally is running. This ensures
 * that the {@link MetricUsageCollector} will only need to initialize the usage calculation from the
 * DB if it is not already in the cache, avoiding the need to loading the same calculation for each
 * batch of events.
 */
@Slf4j
