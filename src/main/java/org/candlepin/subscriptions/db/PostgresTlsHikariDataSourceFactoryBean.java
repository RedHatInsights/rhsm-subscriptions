...
/**
 * Class to create the HikariDataSource. Highly PostgreSQL specific. Other databases will require
 * their own FactoryBean.
 */
public class PostgresTlsHikariDataSourceFactoryBean extends AbstractFactoryBean<HikariDataSource>
  ...
  protected HikariDataSource createInstance() throws Exception {
    ...
    // If you set "ssl" to any value at all, the JDBC driver will try to connect over TLS, so do not
    // add
    // any properties at all if 'enable-tls' isn't true.
    if ("postgresql".equalsIgnoreCase(tlsProperties.getPlatform()) && tlsProperties.isEnableTls()) {
      ...
      dataSource.addDataSourceProperty(
          "sslrootcert", getRootCaFile(tlsProperties.getRootCaLocation()));
    }
  ...
  /**
   * Method to return the canonical path to the CA file. Any overriding implementation should fail
   * fast on a read error by throwing an exception; it should not return null/empty string as that
   * will only obscure the actual issue.
   *
  ...
  protected String getRootCaFile(String rootCaLocation) {
    ...
    try {
      ...
      // If we can't read the CA, just die right now.  Any further error messages will only obscure
      // the
      // true problem
  ...
  public void setTlsDataSourceProperties(
      PostgresTlsDataSourceProperties postgresTlsDataSourceProperties) {
