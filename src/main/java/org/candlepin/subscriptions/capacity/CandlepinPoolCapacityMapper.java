...
public class CandlepinPoolCapacityMapper {
   ...
   *
   * <p>If existing capacity records are passed, then they are updated, otherwise new
   * SubscriptionCapacity instances are created.
   *
   ...
   */
  public Collection<SubscriptionCapacity> mapPoolToSubscriptionCapacity(
      String ownerId,
      CandlepinPool pool,
  ...
  private Long getCapacityUnit(String unitProperty, CandlepinPool pool) {
    Integer units =
        pool.getProductAttributes().stream()
            .filter(attr -> attr.getName().equals(unitProperty))
            .map(CandlepinProductAttribute::getValue)
  ...
  private ServiceLevel getSla(CandlepinPool pool) {
    Optional<String> sla =
        pool.getProductAttributes().stream()
            .filter(attr -> attr.getName().equals("support_level"))
            .map(CandlepinProductAttribute::getValue)
            .findFirst();
  ...
  private Usage getUsage(CandlepinPool pool) {
    Optional<String> usage =
        pool.getProductAttributes().stream()
            .filter(attr -> attr.getName().equals("usage"))
            .map(CandlepinProductAttribute::getValue)
            .findFirst();
  ...
  private List<String> extractProductIds(Collection<CandlepinProvidedProduct> providedProducts) {
    return providedProducts.stream()
        .map(CandlepinProvidedProduct::getProductId)
        .collect(Collectors.toList());
  }
}
