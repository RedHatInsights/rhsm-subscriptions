...
public class SubscriptionTableController {
    ...
    private SubscriptionTableController(SubscriptionCapacityViewRepository subscriptionCapacityViewRepository, ApplicationClock clock){
    ...
    */
        ...
        List<SubscriptionCapacityView> capacities =
                subscriptionCapacityViewRepository
                        .findByKeyOwnerIdAndKeyProductIdAndServiceLevelAndUsage(ownerId, productId.toString(), sanitizedSla, sanitizedUsage);

        for (SubscriptionCapacityView subscriptionCapacityView : capacities) {
            String sku = subscriptionCapacityView.getSku();
            final SkuCapacity inventory = inventories.computeIfAbsent(sku, key -> initializeDefaultSkuCapacity(subscriptionCapacityView, uom));
    ...
    public SkuCapacity initializeDefaultSkuCapacity(SubscriptionCapacityView subscriptionCapacityView, Uom uom){
        ...
        inv.setServiceLevel(Optional.ofNullable(subscriptionCapacityView.getServiceLevel()).orElse(ServiceLevel.EMPTY).asOpenApiEnum());
        inv.setUsage(Optional.ofNullable(subscriptionCapacityView.getUsage()).orElse(Usage.EMPTY).asOpenApiEnum());
    ...
    public void calculateNextEvent(SubscriptionCapacityView subscriptionCapacityView, SkuCapacity skuCapacity, OffsetDateTime now){
    ...
    public void addSubscriptionInformation(SubscriptionCapacityView subscriptionCapacityView, SkuCapacity skuCapacity){
        ...
        skuCapacity.setQuantity(skuCapacity.getQuantity() + (int) subscriptionCapacityView.getQuantity());
    }

    public void addTotalCapacity(SubscriptionCapacityView subscriptionCapacityView, SkuCapacity skuCapacity){
        ...
        skuCapacity.setTotalCapacity(skuCapacity.getPhysicalCapacity() + skuCapacity.getVirtualCapacity());
