...
class CapacityReconciliationControllerTest {
  ...
  @AfterEach
  void afterEach(){
    reset(subscriptionCapacityRepository,
            capacityProductExtractor,
            offeringRepository,
            whitelist);
  }
  ...
  void shouldAddNewCapacitiesIfNotAlreadyExisting() {
    ...
    Offering offering = Offering.builder()
            .productIds(Arrays.asList(45))
            .sku("MCT3718")
            .build();

    Subscription newSubscription = createSubscription("456", 10);
    Collection<SubscriptionCapacity> capacities = productIds.stream()
    ...
    when(subscriptionCapacityRepository.findByKeySubscriptionId("456")).thenReturn(Collections.emptyList());
  ...
  void shouldUpdateCapacitiesIfThereAreChanges() {
    ...
    Offering updatedOffering = Offering.builder()
    ...
    List<SubscriptionCapacity> existingCapacities = List.of(
            SubscriptionCapacity.builder()
                    .key(SubscriptionCapacityKey.builder().subscriptionId("456").ownerId("123").productId("RHEL").build())
                    .physicalCores(10).physicalSockets(15).build(),
            SubscriptionCapacity.builder()
                    .key(SubscriptionCapacityKey.builder().subscriptionId("456").ownerId("123").productId("RHEL Workstation").build())
                    .physicalCores(10).physicalSockets(15).build());

    List<SubscriptionCapacity> updatedCapacities = productIds.stream()
            .map(productId -> SubscriptionCapacity.from(updatedSubscription, updatedOffering, productId))
            .collect(Collectors.toList());
    ...
    when(subscriptionCapacityRepository.findByKeySubscriptionId("456")).thenReturn(existingCapacities);
  ...
  void shouldNotAddNewCapacitiesWhenProductIsNotOnWhitelist() {
    ...
    when(capacityProductExtractor.getProducts(any())).thenReturn(Set.of("RHEL", "RHEL Workstation"));
    when(offeringRepository.getById("MCT3718")).thenReturn(offering);
    when(subscriptionCapacityRepository.findByKeySubscriptionId("456")).thenReturn(existingCapacities);

    capacityReconciliationController.reconcileCapacityForSubscription(subscription);
    verify(subscriptionCapacityRepository).deleteAll(MockitoHamcrest.argThat(Matchers.containsInAnyOrder(existingCapacities.get(0), existingCapacities.get(1))));
  }
  ...
  void shouldAddNewCapacitiesAndRemoveAllStaleCapacities() {
    ...
    Offering offering = Offering.builder()
            .productIds(Collections.singletonList(45))
            .sku("MCT3718")
            .build();
    Subscription subscription = createSubscription("456", 10);

    List<SubscriptionCapacity> newCapacities = List.of(SubscriptionCapacity.from(subscription, offering, "RHEL"));

    List<SubscriptionCapacity> staleCapacities = List.of(
            SubscriptionCapacity.builder()
                    .key(SubscriptionCapacityKey.builder().subscriptionId("456").ownerId("123").productId("STALE RHEL").build())
                    .physicalCores(10).physicalSockets(15).build(),
            SubscriptionCapacity.builder()
                    .key(SubscriptionCapacityKey.builder().subscriptionId("456").ownerId("123").productId("STALE RHEL Workstation").build())
                    .physicalCores(10).physicalSockets(15).build());
    ...
    verify(subscriptionCapacityRepository).deleteAll(MockitoHamcrest.argThat(Matchers.containsInAnyOrder(staleCapacities.get(0), staleCapacities.get(1))));
  }
