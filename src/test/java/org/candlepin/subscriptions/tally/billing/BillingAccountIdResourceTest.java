...
class BillingAccountIdResourceTest {
  ...
  @Mock private HostTallyBucketRepository hostTallyBucketRepository;
  private BillingAccountIdResource resource;

  @BeforeEach
  public void setupTests() {
    when(orgConfigRepository.existsByOrgId("owner123456")).thenReturn(true);
    resource = new BillingAccountIdResource(hostTallyBucketRepository);
  }
  ...
  void testAccessAllowedWithPsk() {
    BillingAccountIdResponse reponse =
        resource.fetchBillingAccountIdsForOrg(
            "owner789", RHEL_FOR_ARM.getValue(), BillingProvider.AWS.getValue());
    assertThat(reponse.getIds()).hasSize(0);
  }
