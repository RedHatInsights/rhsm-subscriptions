...
class CapacityResourceTest {
  ...
  @Test
  void testValidateGranularityWhenProductIdNotConfigured() {

    var thrownException =
        Assertions.assertThrows(
            BadRequestException.class,
            () -> resource.validateGranularity(SATELLITE, Granularity.MONTHLY));

    assertEquals(
        String.format("No granularity configuration found for %s", SATELLITE),
  ...
  void testValidateGranularityIncompatible() {
    ...
    assertEquals(
        String.format(
            "%s does not support any granularity finer than %s", RHEL_FOR_ARM, Granularity.HOURLY),
        thrownException.getMessage());
  }
