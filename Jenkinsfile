        ...
        stage('Build/Test/Lint') {
            steps {
                script {
                    ...
                    exitCode = sh(script: "export GRADLE_OPTS='-Dorg.gradle.daemon=false' && ./gradlew test --no-build-cache --no-daemon --info --stacktrace testCodeCoverageReport -DexcludeTags=integration", returnStatus: true)
                    if (exitCode != 0) {
