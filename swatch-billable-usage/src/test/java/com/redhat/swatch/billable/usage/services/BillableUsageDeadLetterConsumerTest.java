...
class BillableUsageDeadLetterConsumerTest {
  ...
  private OffsetDateTime expectedRetryAfter;
  ...
  private BillableUsageRemittanceEntity givenExistingRemittanceEntityForUsage() {
    ...
    when(billableUsageRemittanceRepository.findById(usage.getUuid())).thenReturn(entity);
    return entity;
  }
        ...
        Message.of(usage)
            .addMetadata(
                OutgoingKafkaRecordMetadata.<String>builder()
                    .withHeaders(
                        new RecordHeaders()
                            .add(RETRY_AFTER_HEADER, expectedRetryAfter.toString().getBytes()))
                    .build()));
  }
  ...
  private void thenEntityShouldBeUpdated(BillableUsageRemittanceEntity entity) {
    ...
    assertEquals(expectedRetryAfter, entity.getRetryAfter());
  }
