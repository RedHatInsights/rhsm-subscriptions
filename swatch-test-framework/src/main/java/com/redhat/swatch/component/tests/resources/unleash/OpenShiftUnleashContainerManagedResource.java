...
public class OpenShiftUnleashContainerManagedResource extends OpenShiftContainerManagedResource {
  ...
  public void start() {
    super.start();
    var client = getOpenShiftClient();
    context
        .getOwner()
        .withProperty(
            UnleashService.ADMIN_TOKEN,
            client.getSecretValue(
                "env-" + client.namespace() + "-featureflags", "adminAccessToken"));
  }
  ...
  protected String serviceName() {
    if (serviceName == null) {
      ...
      if (services.isEmpty()) {
        ...
        Log.error(
            context.getOwner(),
            "Too many services found with labels: " + LABELS + ". Expected only one.");
        throw new ServiceNotFoundException(NAME);
      }
