...
public interface AnnotationBinding {
  ...
  default <T extends Annotation> Optional<T> findAnnotation(
      Annotation[] annotations, Class<T> clazz) {
    return Stream.of(annotations).filter(clazz::isInstance).map(a -> (T) a).findFirst();
  }

  /**
   * Return the default service implementation for the current annotation. Used for annotations that
   * are used at class level.
   */
  ...
  /**
   * Return the default service name for the current annotation. Used for annotations that are used
   * at class level.
   */
