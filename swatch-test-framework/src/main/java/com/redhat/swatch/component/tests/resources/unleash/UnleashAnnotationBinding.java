...
public class UnleashAnnotationBinding implements AnnotationBinding {

  @Override
  public boolean isFor(Annotation... annotations) {
    return findAnnotation(annotations, Unleash.class).isPresent();
  }

  @Override
  public ServiceLifecycle getLifecycle(Annotation annotation) {
    return ((Unleash) annotation).lifecycle();
  }
  ...
  public ManagedResource getManagedResource(
    ...
    if (OpenShiftExtensionBootstrap.isEnabled(context)) {
      return new OpenShiftUnleashContainerManagedResource();
    }

    // If none handler found, then the container will be running on localhost by default
    return new LocalUnleashManagedResource();
  }
}
