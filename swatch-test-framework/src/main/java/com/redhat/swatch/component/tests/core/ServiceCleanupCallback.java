...
class ServiceCleanupCallback implements ExtensionContext.Store.CloseableResource {
  ...
  ServiceCleanupCallback(Service service, String serviceName) {
  ...
  public void close() throws Throwable {
    // Only stop if still running (services may have been stopped manually in afterAll)
    if (service.isRunning()) {
      Log.info("JVM Shutdown: stopping service %s", serviceName);
      service.stop();
    }
  }
}
