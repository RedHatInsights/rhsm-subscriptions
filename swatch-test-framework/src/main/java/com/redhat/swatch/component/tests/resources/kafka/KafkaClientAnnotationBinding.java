 ...
 */
package com.redhat.swatch.component.tests.resources.kafka;
...
public class KafkaClientAnnotationBinding implements AnnotationBinding {

  @Override
  public boolean isFor(Annotation... annotations) {
    return findAnnotation(annotations, KafkaClient.class).isPresent();
  }
  ...
  public ManagedResource getManagedResource(
    ...
    if (OpenShiftExtensionBootstrap.isEnabled(context)) {
      return new OpenShiftKafkaClientManagedResource();
    }

    // If none handler found, then the container will be running on localhost by default
    return new LocalKafkaClientManagedResource();
  }
}
