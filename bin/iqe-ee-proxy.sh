...
oc port-forward $(oc get pod -o name | grep featureflags | grep -v featureflags-db | grep -v featureflags-edge) 4242:4242 &
...
oc port-forward $(oc get service -o name | grep prometheus-service) 9090:8000 &
