...
@ApplicationScoped
public class IqeTestMdcOutgoingInterceptor implements OutgoingInterceptor {
  @Override
  public Message<?> beforeMessageSend(Message<?> message) {
    String iqeTest = MDC.get(Constants.IQE_TEST_HEADER);
    if (!StringUtil.isNullOrEmpty(iqeTest) && message instanceof KafkaMessage kafkaMessage) {
      kafkaMessage.getHeaders().put(Constants.IQE_TEST_HEADER, iqeTest);
    }
