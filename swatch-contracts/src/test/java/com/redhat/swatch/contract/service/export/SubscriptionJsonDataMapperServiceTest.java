...
@QuarkusTest
public class SubscriptionJsonDataMapperServiceTest {
  ...
  public void testMapDataItem() {
    ...
    assertEquals(subscriptionNumber, jsonItem.getSubscriptionNumber());
    assertEquals(sku, jsonItem.getSku());
    assertEquals(productName, jsonItem.getProductName());
    assertEquals(10.0, jsonItem.getQuantity());
    assertEquals(billingAccountId, jsonItem.getBillingAccountId());
    assertEquals(aws.getValue(), jsonItem.getBillingProvider());
    ...
    assertEquals(capacity, measurement.getCapacity());
    assertEquals(Usage.PRODUCTION.getValue(), jsonItem.getUsage());
    assertEquals(ServiceLevel.PREMIUM.getValue(), jsonItem.getServiceLevel());
  }
}
