...
class PskHeaderAuthenticationMechanismTest {
  ...
  @Test
  void testPskAuthenticationNotAttemptedWhenHeaderMissing() {
    var auth = new PskHeaderAuthenticationMechanism();
    var subscriber =
  ...
  @Test
  void testPskAuthenticationRequestedWhenHeaderPresent() {
    var auth = new PskHeaderAuthenticationMechanism();
    when(identityProviderManager.authenticate(any()))
        .thenReturn(Uni.createFrom().item(authResponse));
    var subscriber =
        auth.authenticate(
                mockRequest(Map.of("x-rh-swatch-psk", "placeholder")), identityProviderManager)
            .subscribe()
            .withSubscriber(UniAssertSubscriber.create());
    subscriber.assertCompleted().assertItem(authResponse);
    verify(identityProviderManager).authenticate(new PskAuthenticationRequest("placeholder"));
  }
}
