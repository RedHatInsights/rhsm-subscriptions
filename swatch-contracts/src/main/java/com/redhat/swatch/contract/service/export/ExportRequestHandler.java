...
import com.redhat.swatch.contract.service.export.utils.FileUtils;
import java.io.File;
...
public class ExportRequestHandler {
  ...
  private void uploadData(DataExporterService<?> exporterService, ExportServiceRequest request) {
    ExportFileWriter writer = getFileWriter(request);
    File file = createTemporalFile(request);
    try (var data = exporterService.fetchData(request)) {
      writer.write(file, exporterService.getMapper(request), data, request);
      log.info(
