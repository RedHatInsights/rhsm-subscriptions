...
public class CustomerOverUsageService {
  ...
  protected static final String OVER_USAGE_METRIC = "swatch_utilization_over_usage";
  ...
  public void check(UtilizationSummary payload) {
    // Only check summaries with DAILY or HOURLY granularity
    if (!isMatchingGranularity(payload)) {
      log.debug(
          "Skipping over-usage check for orgId={} productId={} with granularity={}. Expected: DAILY or HOURLY",
          payload.getOrgId(),
          payload.getProductId(),
          payload.getGranularity());
      return;
    }
  ...
  private void checkMeasurement(
    ...
    if (shouldSkipMeasurement(measurement)) {
      return;
    }
  ...
  /**
   * Determines if a measurement should be skipped (unlimited or invalid capacity/usage).
   *
   * @param measurement the measurement to check
   * @return true if the measurement should be skipped
   */
  private boolean shouldSkipMeasurement(Measurement measurement) {
    return Boolean.TRUE.equals(measurement.getUnlimited())
  ...
  private boolean isMatchingGranularity(UtilizationSummary payload) {
    if (payload.getGranularity() == null) {
      return false;
    }
    UtilizationSummary.Granularity granularity = payload.getGranularity();
    return granularity == UtilizationSummary.Granularity.DAILY
        || granularity == UtilizationSummary.Granularity.HOURLY;
  }
