...
public enum Product {
  ...
  public Metric getFirstMetric() {
    return this.metrics.values().stream().findFirst().orElseThrow();
  }

  public MetricId getFirstMetricId() {
    return MetricId.fromString(getFirstMetric().getId());
  }

  public static Product fromString(String name) {
    ProductId id = ProductId.fromString(name);
    return Stream.of(Product.values()).filter(p -> p.getId().equals(id)).findFirst().orElse(null);
  }
}
