...
public class CustomerOverUsageComponentTest extends BaseUtilizationComponentTest {
  ...
  void initializeCount() {
    initialCount = service.getMetricByTags(OVER_USAGE_METRIC, orgIdTag(orgId));
  }
  ...
  void shouldIncrementOverUsageCounter_whenUsageExceedsThreshold() {
    ...
    thenCounterShouldBeIncremented();
  }
  ...
  void shouldIncrementCounterOnlyForMeasurementsExceedingThreshold() {
    ...
    AwaitilityUtils.untilAsserted(
        () -> {
          double currentCount = service.getMetricByTags(OVER_USAGE_METRIC, orgIdTag(orgId));
          assertThat(
  ...
  // Helper methods
  private String orgIdTag(String orgId) {
    return String.format("org_id=\"%s\"", orgId);
  }
  ...
  // Then helpers
  void thenCounterShouldBeIncremented() {
    AwaitilityUtils.untilAsserted(
        () -> {
          double currentCount = service.getMetricByTags(OVER_USAGE_METRIC, orgIdTag(orgId));
          assertThat(
              "Over-usage counter should be incremented", currentCount, greaterThan(initialCount));
        });
  }

  void thenCounterShouldNotChange() {
